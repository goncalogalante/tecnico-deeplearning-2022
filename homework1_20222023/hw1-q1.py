...
class MLP(object):
    ...
    def train_epoch(self, X, y, learning_rate=0.001):
        ...
        for x_i, index_y_i in zip(X, y):
            x_i = np.asarray(x_i)
            index_y_i = np.asarray(index_y_i)
            y_i = np.zeros(self.outl_b.shape[0])
            y_i[index_y_i] = 1
                
            # h and predict calculation
            z = self.hiddenl_W.dot(x_i) + self.hiddenl_b 
            h = ReLu(z) # h = g(z)
            p = Softmax(self.outl_W.dot(h) + self.outl_b) # output = o(W_output*h + b_output)
                
            # back propagation in output layer
            grad_output_layer = -y_i + p # grad_output = -y + p 
            grad_outl_w = grad_output_layer[:,None].dot(h[None,:]) # dL/dW (output)
            grad_outl_b = grad_output_layer # cldL/db (output)
            
            # back propagation in hidden layer
            grad_hidden_layer = self.outl_W.T.dot(grad_output_layer)
            grad_relu = ReLu_grad(h)*grad_hidden_layer
            grad_hiddenl_w = grad_relu[:, None].dot(x_i[None,:]) # dL/dW (hidden)
            grad_hiddenl_b = grad_relu # dL/db (hidden)
