...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        self.number_layers = layers
        
        if self.number_layers == 1:
            self.linear_layer1 = nn.Linear(n_features, hidden_size) # input -> hidden layer
            #self.linear_layer2 = nn.Linear(hidden_size, hidden_size) # hidden layer -> hidden layer
            #self.linear_layer3 = nn.Linear(hidden_size, hidden_size) # hidden layer -> hidden layer
            self.linear_outputlayer = nn.Linear(hidden_size, n_classes) # hidden layer -> output
            self.Dropout = nn.Dropout(dropout)
        
        elif self.number_layers == 2:
            self.linear_layer1 = nn.Linear(n_features, hidden_size) # input -> hidden layer
            self.linear_layer2 = nn.Linear(hidden_size, hidden_size) # hidden layer -> hidden layer
            #self.linear_layer3 = nn.Linear(hidden_size, hidden_size) # hidden layer -> hidden layer
            self.linear_outputlayer = nn.Linear(hidden_size, n_classes) # hidden layer -> output
    ...
    def forward(self, x, **kwargs):
        ...
        if self.number_layers == 1:
            x = self.linear_layer1(x) # input -> hidden layer
            x = self.activation(x) # activation function
            x = self.Dropout(x) # dropout
            x = self.linear_outputlayer(x) # hidden layer -> output
            return x
